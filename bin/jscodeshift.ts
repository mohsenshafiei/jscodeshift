...
import requirePackage from "../utils/requirePackage";
...
const packageInfo: PackageJson = pkg;

import { DEFAULT_EXTENSIONS } from "@babel/core";
import { Options } from "../src/types/core.js";
...
const parser = argsParser.options({
  ...
  parserConfig: {
    ...
    metavar: "FILE",
    process: (file: string) => JSON.parse(fs.readFileSync(file).toString()),
  },
  failOnError: {
    display_index: 4,
    flag: true,
  ...
  version: {
    ...
    callback: function () {
      return [
        `jscodeshift: ${packageInfo.version}`,
        ` - babel: ${babelCore.version}`,
        // @ts-ignore
        ` - babylon: ${babelParser.version}`,
        // @ts-ignore
        ` - flow: ${flowParser.version}`,
        // @ts-ignore
...
function run(paths: (string | number)[], options: Options) {
  Runner(
