...
// @ts-ignore
function run(args?: string[], stdin?: string, cwd?: string): Promise<string[]> {
  return new Promise((resolve) => {
    // @ts-ignore
    const jscodeshift = spawn(path.join(__dirname, "../jscodeshift.js"), args, {
      cwd,
    });
    let stdout = "";
    let stderr = "";
...
describe("jscodeshift CLI", () => {
  it.only("calls the transform and file information", () => {
    ...
    const transformB = createTransformWith("return fileInfo.path;");
    ...
    return Promise.all([
      ...
      run(["-t", transformB, sourceC]).then((out: string[]) => {
        expect(out[1]).toBe("");
  ...
  describe("Babel", () => {
    ...
    it("supports ES6 features in transform files", () => {
      const sourceA = createTempFileWith("a", "sourceA", ".js");
      const transform = createTransformWith("const a = 42; return a;");
      return Promise.all([
        run(["-t", transform, sourceA]).then(() => {
          expect(readFile(sourceA).toString()).toEqual("42");
        }),
      ]);
    });
    ...
    it("supports class properties in transform files", () => {
      ...
      return Promise.all([
        run(["-t", transform, sourceA]).then(() => {
          expect(readFile(sourceA).toString()).toMatch(/\(this,\s*['"]x['"]/);
        }),
      ]);
    });
    ...
    it("supports flow type annotations in transform files", () => {
      ...
      return Promise.all([
        run(["-t", transform, sourceA]).then(() => {
          expect(readFile(sourceA).toString()).toMatch(/a\s*=\s*42/);
        }),
      ]);
    });
    ...
    it("supports Typescript type annotations in transform files", () => {
      ...
      return Promise.all([
        run(["-t", transform, sourceA]).then((args) => {
          expect(readFile(sourceA).toString()).toMatch(
            /function\s+foo\(x\)\s*{}/
          );
        }),
      ]);
    ...
    it("transpiles imported Typescript files in transform files", () => {
      ...
      return Promise.all([
        run(["-t", transform, sourceA]).then((args) => {
          expect(readFile(sourceA).toString()).toMatch(/function\s*\(x\)\s*{}/);
        }),
      ]);
    });
  });
  ...
  describe("ignoring", () => {
    ...
    it("sources ignore patterns from a root directory's .gitignore file", () => {
      ...
      return run(["-t", transform, "--gitignore"].concat(sources))
        .then(() => {
          expect(readFile(sources[0]).toString()).toBe("transforma");
          expect(readFile(sources[1]).toString()).toBe("a");
          expect(readFile(sources[2]).toString()).toBe("subfile");
    ...
    it("accepts a list of configuration files", () => {
      const gitignore = createTempFileWith(
        ["sub/dir/"].join("\n"),
        ".gitignore"
      );
      const eslintignore = createTempFileWith(
        ["**/*test.js", "sourceA.js"].join("\n"),
        ".eslintignore"
      );
      const configs = [
        "--ignore-config",
  ...
  describe("output", () => {
    it("shows workers info and stats at the end by default", () => {
      ...
      return run(["-t", transform, sourceA]).then((out: string[]) => {
    ...
    it("does not ouput anything in silent mode", () => {
      const sourceA = createTempFileWith("a", "sourceA", ".js");
      const transform = createTransformWith("return null;");
      return run(["-t", transform, "-s", sourceA]).then((out: string[]) => {
  ...
  describe("--parser-config", () => {
    it("allows custom parser settings to be passed", () => {
      ...
      const parserConfig = createTempFileWith(
        JSON.stringify({
          sourceType: "module",
          tokens: true,
