...
describe("argsParser", function () {
    ...
    function test(testCases: any) {
      for (const testName in testCases) {
        ...
        it(testName + " (space separated values)", function () {
          const parse = () =>
            parser.parse(Array.prototype.concat.apply([], testCase.args));
        ...
        it(testName + " (= separated values)", function () {
          const parse = () =>
            parser.parse(testCase.args.map((args: any) => args.join("=")));
          if (typeof testCase.expected === "string") {
            expect(parse).toThrowError(testCase.expected);
          } else {
            expect(parse()).toEqual(testCase.expected);
  ...
  describe("flags", function () {
    ...
    it("sets values to true of specified", function () {
      expect(parser.parse(["--foo", "--another-bar", "foo", "bar"])).toEqual({
        options: {
          foo: true,
          bar: true,
        },
    ...
    it("understands short options", function () {
      expect(parser.parse(["-f", "--another-bar", "f", "bar"])).toEqual({
        options: {
          foo: true,
          bar: true,
        },
    ...
    it("sets default value if not specified", function () {
      expect(parser.parse(["f", "bar"])).toEqual({
        options: {
          bar: false,
        },
        positionalArguments: ["f", "bar"],
    ...
    it("accepts flag=0 and flag=1 (undocumented)", function () {
      expect(parser.parse(["--foo=0", "--another-bar=1"])).toEqual({
        options: {
          foo: false,
          bar: true,
        },
      ...
      expect(parser.parse(["-f=0"])).toEqual({
        options: {
          foo: false,
          bar: false,
        },
    ...
    it("understands --no-prefixes", function () {
      expect(parser.parse(["--no-foo", "--no-another-bar"])).toEqual({
        options: {
          foo: false,
          bar: false,
        },
