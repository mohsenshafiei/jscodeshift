...
describe("VariableDeclarators", function () {
  ...
  describe("Traversal", function () {
    ...
    it("finds variable declarators by name", function () {
      const declarators =
        Collection.fromNodes(nodes).findVariableDeclarators("bar");
  ...
  describe("Transform", function () {
    ...
    it("properly renames a shorthand property that was using the old variable name", async function () {
      ...
      expect(
        Collection.fromNodes(nodes).find(types.Identifier, { name: "foo" })
          .length
      ).toBe(1);
      ...
      expect(
        Collection.fromNodes(nodes)
          .find(types.Property)
          .filter((prop: any) => !prop.value.shorthand).length
      ).toBe(1);
      expect(
        Collection.fromNodes(nodes)
          .find(types.Property)
    ...
    it("does not rename React component prop name", function () {
      ...
      const identifiers = Collection.fromNodes(nodes).find(
        types.JSXIdentifier,
        { name: "foo" }
      );
